...
def register_auth_routes(app, db_pool):
    ...
    def signup():
        """Register the student/user to InsightQA Platform"""
        try:
            # Validation of data received in the request
            data = request.get_json()
            if not data:
                return jsonify( { "error": "No data provided" } ), 400
            if not all(key in data for key in ("email", "password", "name", "dept_name")):
            ...
            with get_db_connection(db_pool) as connection:
                with connection.cursor() as cursor:
                    ...
                    query = "insert into students (email, name, password, dept_name, verification_token, token_expiration) values (%s, %s, %s, %s, %s, %s)"
        ...
        try:
            with get_db_connection(db_pool) as connection:
                with connection.cursor() as cursor:
                    ...
                    cursor.execute("select email from students where verification_token = %s and token_expiration > %s", (token, datetime.now()))
                    user = cursor.fetchone()
                    if not user:
                        return jsonify( { "error": "Invalid or expired verification token" } ), 400
                    # Update the verification status of the user in the database
                    query = "update students set is_verified = 1, verification_token = NULL, token_expiration = NULL where verification_token = %s"
                    data = (token, )
        ...
        try:
            ...
            with get_db_connection(db_pool) as connection:
                with connection.cursor() as cursor:
                    ...
                    decrypted_password = decrypt(student["password"])
